...
resq_angel <- read_excel("resq-raw-data/angel_targets.xlsx") 
...
dt <- data.frame(patients = c( 42,71,255,489,753), #these values are also made up, except for the last one, which is the res-q patient count
...
Annotations with ggplot test
```{r angel, echo = FALSE}
angel_annotations_needle <- data.frame(
  x = c(2019, 2019),
  y = c(60, 45),
  label = c("< 60","< 45")
)

p_and <- ggplot(resq_merge[resq_mort$door_to_needle<99 & resq_mort$discharge_date<2201,], 
            aes(discharge_date, door_to_needle))
p_and + geom_point() +  geom_label(data = angel_annotations_needle, aes(x=x, y=y, label = label))

```

Should try comparing individual data points for door to needle and MRSdischarge.
So, MRSdisharge on Y axis, door to needle time on x-axis.
```{r mrsAndDoorToNeedle, echo = FALSE}
p_MRS_DTN_full <- ggplot(resq_mort[resq_mort$door_to_needle<99 & resq_mort$discharge_date<2201 & resq_mort$discharge_mrs<99,],
                         aes(door_to_needle, discharge_mrs))
p_MRS_DTN_full + geom_smooth() + geom_point()
```
