...
resq_mort<-resq_merge %>%
  select(subject_id, hospital_days, DistDestStr) %>%
  group_by(subject_id)%>%
  filter(DistDestStr == "Dead") %>%
  mutate(
    within30DayMort = if_else(hospital_days <= 30, "Dead", "Alive") 
  ) 

resq_merge<-resq_merge %>%
  left_join(resq_merge, resq_mort, by="subject_id")
