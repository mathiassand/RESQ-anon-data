...
resq_merge<-resq_merge %>%
  mutate(
    within30DayMort = if_else(hospital_days > 30 | (hospital_days < 30 &DistDestStr != "Dead"), 0, 1), 
  ) 
  
df<-resq_merge
df %>% dplyr::group_by(discharge_date) %>% summarise(Mortality30d =mean(within30DayMort))
