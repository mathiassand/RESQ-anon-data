...
testText <- grobTree(textGrob("Annotation testing", x=0.10, y=0.40, hjust=0, gp=gpar(col="grey", fontsize=13, fontface="italic")))
...
aes(x = discharge_date, y = Mortality30dMean)) + 
  ...
  xlab("Discharge date") +
  expand_limits(y=0) +
  geom_text(aes(label=round(Mortality30dMean, digits = 2)), vjust = 2.5, color = "#0F85C8" ) +
  labs(
...
aes(y = Mortality30dMean, x = DTNMean)) + 
  ...
  geom_text(aes(label=round(Mortality30dMean, digits=2)), vjust = 2.5, color = "#0F85C8" ) +
    labs(
    title = "Mortality rate against <span style = 'color: #0F85C8;'>Door to needle</span> evolution"
  ) +
  theme(
    plot.title = element_markdown(),
...
aes(y = Mortality30dMean, x = DTGMean, color="Mean")) + 
  ...
  xlab("Door to groin") +
  expand_limits(y=0) +
  geom_text(aes(label=round(Mortality30dMean, digits=2)), vjust = 2.2, hjust = 0, color = "#0F85C8" ) +
    labs(
    title = "Mortality rate against <span style = 'color: #0F85C8;'>Door to groin</span> evolution"
  ) +
  theme(
    plot.title = element_markdown(),
...
aes(y = Mortality30dMean, x = DISMRSMean, color="Mean")) + 
  ...
  geom_text(aes(label=round(Mortality30dMean, digits=2)), vjust = 2.5, color = "#0F85C8" ) +
    labs(
    title = "Mortality rate against <span style = 'color: #0F85C8;'>Discharge mrs</span> evolution"
  ) +
  theme(
    plot.title = element_markdown(),
    axis.title.x = element_text(hjust=0.14, vjust=0.2),
    axis.title.y = element_text(hjust=0.77, vjust=0.77),
    plot.caption = element_text(hjust=0, size=6),
    ) +
  annotation_custom(testText)
